...
if (!function_exists('formatPhone')) {
    function formatPhone($number, $format = 'international')
    {
        $number = preg_replace('/\D/', '', $number); // Remove non-digits

        if (preg_match('/^0[17]\d{8}$/', $number)) {
            return ($format === 'international') ? '254' . substr($number, 1) : $number;
        }

        if (preg_match('/^254[17]\d{8}$/', $number)) {
            return ($format === 'local') ? '0' . substr($number, 3) : $number;
        }
...
if (!function_exists('message_template')) {
    function message_template($key, $replacements = []) {
        return \App\Helpers\MessageHelper::get($key, $replacements);
    }
}
